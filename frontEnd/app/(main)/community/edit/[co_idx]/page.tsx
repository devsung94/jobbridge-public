...
export default function CommunityEditPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert("게시글이 수정되었습니다.");
      router.push(`/community/${co_idx}${backUrl}`);
      // router.replace(`/community/${co_idx}`);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        ...
        if (data.result === "Y") {
          ...
          const categoryOption = categoryOptions.find(opt => opt.value === post.category) || categoryOptions[0];
          setForm({
            userId: post.userId,
    ...
    fetchPost();
  }, [isLoading, isLoggedIn, role, userId, co_idx, router]);
